...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if (pindexBest->nHeight < 5 ) // premine
    {
    nSubsidy = 100000000 * COIN;
    return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight <= 100) // 0 coins
    {
      nSubsidy = 0 * COIN;
    return nSubsidy + nFees;
    }
    ...
    else if (pindexBest->nHeight <= 20000) // end of POW
    {
      nSubsidy = 1000 * COIN;
    return nSubsidy + nFees;
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    nRewardCoinYear = COIN_YEAR_REWARD; // 10,000%
    int64_t nSubsidy = 0 * COIN;
    ...
    {
        nSubsidy = nCoinAge * nRewardCoinYear / 365; // 10k%
    }
    else if (pindexBest->nHeight <= 40000)
    {
        nSubsidy = nCoinAge / 2 * nRewardCoinYear / 365; // 5k%
    }
    else if (pindexBest->nHeight <= 50000)
    {
        nSubsidy = nCoinAge / 4 * nRewardCoinYear / 365; // 2.5k%
    }
    else if (pindexBest->nHeight <= 60000)
    {
        nSubsidy = nCoinAge / 8 * nRewardCoinYear / 365; // 1.25k%
    }
    else if (pindexBest->nHeight <= 70000)
    {
        nSubsidy = nCoinAge / 16 * nRewardCoinYear / 365; // ~612%
    }
    else if (pindexBest->nHeight <= 80000)
    {
        nSubsidy = nCoinAge / 32 * nRewardCoinYear / 365; // ~308 %
    }
    else if (pindexBest->nHeight <= 90000)
    {
        nSubsidy = nCoinAge / 64 * nRewardCoinYear / 365; // ~156%
    }
    else if (pindexBest->nHeight > 90000)
    {
        nSubsidy = nCoinAge / 128 * nRewardCoinYear / 365; // ~78%
    }
   
    if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfStakeReward(): create=%s nCoinAge=%"PRId64"\n", FormatMoney(nSubsidy).c_str(), nCoinAge);

    return max(nSubsidy, 1 * COIN) + nFees;
}
...
// wow: attempt to generate suitable proof-of-stake
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "China purrs over white tiger triplets";
        CTransaction txNew;
        txNew.nTime = 1539593239;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1539593239;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 4421419;
		if(fTestNet)
        {
            block.nNonce   = 344630;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x45eb428d247869198d33846c0721fe506590a3b5812e56b4e027300d18ef7e01"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x4e, 0x2a, 0x3d, 0x49 };
